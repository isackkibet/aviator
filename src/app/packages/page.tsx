...
function PhoneInputBlock({
  ...
  return (
    <div id="phone-capture-block" className="max-w-xl mx-auto mb-16">
      <div className="fade-up fade-up-2 glass-strong rounded-3xl border border-[#8b5cf6]/15 p-8">
        <div className="text-center mb-6">
          <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-[#8b5cf6] to-violet-700 flex items-center justify-center mx-auto mb-4 transition-all duration-300 hover:scale-110 hover:shadow-lg hover:shadow-[#8b5cf6]/30">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <input
            ...
            className="flex-1 bg-black/30 border border-gray-700/50 rounded-xl px-4 py-3.5 text-white placeholder:text-gray-500 outline-none focus:border-[#8b5cf6] focus:shadow-lg focus:shadow-[#8b5cf6]/10 transition-all duration-300"
          />
          <button
            type="button"
            disabled={!canSave}
            ...
            className="btn-glow btn-glow-purple bg-gradient-to-r from-[#8b5cf6] to-violet-600 text-black px-8 py-3.5 rounded-xl text-sm font-black shadow-2xl transition-all duration-300 disabled:opacity-40 disabled:cursor-not-allowed whitespace-nowrap"
          >
            Save Number
          </button>
        </div>
  ...
  return (
    <>
      ...
      <div className="min-h-screen py-24 px-4 bg-[#0a0e17] aviator-grid-bg relative overflow-hidden">
        ...
        <div className="absolute bottom-[-10%] right-[-5%] w-[400px] h-[400px] rounded-full bg-[#8b5cf6]/5 blur-[120px] mesh-orb-2" />

        <div className="max-w-6xl mx-auto relative z-10">
          {/* Header */}
          <div className="text-center mb-16">
            <div className="fade-up fade-up-1 inline-flex items-center gap-2 px-5 py-2 rounded-full border border-[#8b5cf6]/30 bg-[#8b5cf6]/10 mb-6">
              <span className="w-2 h-2 rounded-full bg-[#8b5cf6] animate-pulse" />
              <span className="text-sm font-bold text-[#8b5cf6]">SECURE PAYMENT VIA PAYSTACK</span>
            </div>
            <h1 className="fade-up fade-up-2 text-6xl font-black mb-4">
              <span className="gradient-text-red">BUY AVIATOR</span>{' '}
              <span className="gradient-text-purple">SIGNALS</span>
            </h1>
          ...
          <div className="grid md:grid-cols-3 gap-8 max-w-5xl mx-auto mb-20">
            {packages.map((pkg, i) => {
              ...
              const cardColors = {
                green: { border: 'border-[#8b5cf6]/15', glow: 'card-glow', text: 'text-[#8b5cf6]', bg: 'from-[#8b5cf6] to-violet-700' },
                    ...
                    <ul className="space-y-3 mb-8">
                      {pkg.features.map((f, fi) => (
                        <li key={fi} className="flex items-center text-gray-300 text-sm">
                          <span className="w-5 h-5 bg-[#8b5cf6]/20 rounded-full flex items-center justify-center mr-3 shrink-0">
                            <svg className="w-3 h-3 text-[#8b5cf6]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg>
                          </span>
                          {f}
                        </li>
                      ))}
          ...
          <div className="fade-up fade-up-6 max-w-4xl mx-auto">
            <div className="glass-strong rounded-3xl border border-[#8b5cf6]/10 p-10">
              <h3 className="text-2xl font-black text-center text-[#8b5cf6] mb-8">HOW PAYMENT WORKS</h3>
              <div className="grid md:grid-cols-4 gap-6">
                {[
                  ...
                  <div key={i} className="text-center">
                    <div className="w-12 h-12 rounded-xl bg-[#8b5cf6]/10 flex items-center justify-center mx-auto mb-3">
                      <svg className="w-6 h-6 text-[#8b5cf6]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={s.icon} /></svg>
                    </div>
                    <div className="text-xs text-[#8b5cf6] font-black mb-1">STEP {s.step}</div>
